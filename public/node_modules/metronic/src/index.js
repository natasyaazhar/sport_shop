...
function recordUtilization( api, config, interval, metadata ) {
	...
	recordMetric(
		...
		'bytes',
		'MB',
		getKey( config, 'physical-used' ),
		proc.rssMB,
		setName( 'PROCESS_RESIDENT_SET' )
	);
	recordMetric(
		api,
		config,
		'bytes',
		'MB',
		getKey( config, 'heap-total' ),
		proc.heapTotalMB,
		setName( 'PROCESS_HEAP_TOTAL' )
	);
	recordMetric(
		api,
		config,
